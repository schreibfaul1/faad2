...
static uint8_t sbr_process_channel(sbr_info* sbr, int32_t* channel_buf, complex_t X[MAX_NTSR][64], uint8_t ch, uint8_t dont_process,
...
uint8_t sbrDecodeSingleFrame(sbr_info* sbr, int32_t* channel, const uint8_t just_seeked, const uint8_t downSampledSBR) {
    ...
    #if(defined(PS_DEC))
uint8_t sbrDecodeSingleFramePS(sbr_info* sbr, int32_t* left_channel, int32_t* right_channel, const uint8_t just_seeked,
                               const uint8_t downSampledSBR) {
    uint8_t   l, k;
    uint8_t   dont_process = 0;
    uint8_t   ret = 0;
